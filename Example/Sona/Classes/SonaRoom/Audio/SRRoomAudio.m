...
- (RACSignal *)startMixedPublish {
    @weakify(self);
    return [[self.sdk.audio stopPullAllId] flattenMap:^RACStream *(id value) {
        @strongify(self);
        return [[self.sdk.audio enter] flattenMap:^RACStream *(id value) {
            @strongify(self);
            return [[self startMultiPublish] flattenMap:^RACStream *(id value) {
                @strongify(self);
...
- (RACSignal *)startMultiPublish {
    @weakify(self);
    return [[self generateStream] flattenMap:^RACStream *(NSString *stream) {
        @strongify(self);
        return [[self.sdk.audio startPublishWithId:stream] flattenMap:^RACStream *(id value) {
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
    ...
    if ([model.roomId isEqualToString:self.configModel.roomId]) {
        if (![self.lastSupplier isEqualToString:model.supplier] ||
            ...
            [[SRGenUserSigSignal genUserSignalWith:@{@"roomId" : self.configModel.roomId}] subscribeNext:^(SRGenUserSigResModel *genRes) {
                [[[[self isPublishingSignal] flattenMap:^RACStream *(NSNumber *res) {
                    BOOL isPublish = [res boolValue];
                    return [[[self.sdk.audio leave] flattenMap:^RACStream *(id value) {
                        @strongify(self)
                        ...
                        return [[self.sdk updateConfigModel:self.configModel] flattenMap:^RACStream *(id value) {
                            @strongify(self)
                            return [[self.sdk.audio enter] flattenMap:^RACStream *(id value) {
                                @strongify(self)
                                [self initConfigPollingService];
                                ...
                                return [[self.sdk.audio startPullAllId] flattenMap:^RACStream *(id value) {
                                    @strongify(self)
...
- (RACSignal *)switchToVideo:(CGSize)size {
    ...
    return [[[SRStreamMixVideoRequestSignal mixedVideo:true
                                                roomId:self.configModel.roomId
                                                  size:size] flattenMap:^RACStream *(id value) {
        return [RACSignal return:@1];
    }] catch:^RACSignal *(NSError *error) {
...
- (RACSignal *)switchToAudio {
    return [[[SRStreamMixVideoRequestSignal mixedVideo:false
                                                roomId:self.configModel.roomId
                                                  size:CGSizeZero] flattenMap:^RACStream *(id value) {
        return [RACSignal return:@1];
    }] catch:^RACSignal *(NSError *error) {
        return [RACSignal error:error];
    }];
